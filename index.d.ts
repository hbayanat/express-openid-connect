...
interface RequestContext {
  ...
  /**
   * Manually update the tokens in the session.
   *
   * ```js
   * app.get('/some-route', async (req, res) => {
   *   const newTokens = await getNewTokensFromSomewhere();
   *   req.oidc.setTokens(newTokens);
   *   ...
   * })
   * ```
   *
   */
  setTokens(tokenSet: Partial<Session>): void;
}
